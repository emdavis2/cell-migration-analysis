...
for region in tracks_geo_region:
    filtered_tracks_geo_region[region] = []
    filtered_tracks_region[region] = []
    for i in range(len(tracks_geo_region[region])):
        trial_df = len(tracks_geo_region[region][i])
        if trial_df < max_tracklength:
            filtered_tracks_geo_region[region].append(tracks_geo_region[region][i])
            filtered_tracks_region[region].append(tracks_region[region][i])
...
open('sentinels/filter_ACF_figures.txt','w').close()
#create new sentinel file to write to
acf_fig_region = open('sentinels/filter_ACF_figures.txt','w')
file_lines = []
...
figure_path = save_path + '/filter_acf_figures'
...
for name in tracks_geo_region:
  ...
  plt.plot(poslagaverage[:-2],label = "positive lag")
  plt.hlines(y=0,xmin=0,xmax=100,color='k')
  plt.xlim(0,min_track_length-4)
  plt.ylim(-0.5,1)
  #plt.errorbar(np.arange(0,min_track_length-4),poslagaverage[0:min_track_length-4],yerr=std_err)
