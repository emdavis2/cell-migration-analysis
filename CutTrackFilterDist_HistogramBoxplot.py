...
for ind,treat_name in enumerate(data_paths):
  ...
  tracks_region[treat_name], tracks_geo_region[treat_name], region_cells[treat_name], region_endpointcells[treat_name] = compile_data_track_cuttracklength(region_data_path, min_track_length, region_list[ind], pixel_size)


#Filter based off distance traveled vs length of cell
filtered_tracks_geo_region = {}
filtered_tracks_region = {}
filtered_region_endpointcells = {}
for treat_name in tracks_geo_region:
  filtered_tracks_geo_region[treat_name] = []
  filtered_tracks_region[treat_name] = []
  filtered_region_endpointcells[treat_name] = pd.DataFrame()
  for i,df in enumerate(tracks_geo_region[treat_name]):
    avg_area = np.average(df['area'].iloc[:,0])
    avg_celllength = 2*np.sqrt(avg_area/np.pi)
    mov = df['movie'][0]
    trackid = df['track_id'][0]
    experiment = df['experiment'][0]
    dist_traveled = region_endpointcells[treat_name].loc[(region_endpointcells[treat_name]['track_id'] == trackid) & (region_endpointcells[treat_name]['movie'] == mov) & (region_endpointcells[treat_name]['experiment'] == experiment)]['D'].iloc[0]
    #print('DIST_TRAVELED', dist_traveled)
    if dist_traveled >= 3*avg_celllength:
      filtered_tracks_geo_region[treat_name].append(df)
      filtered_region_endpointcells[treat_name]=pd.concat([filtered_region_endpointcells[treat_name],region_endpointcells[treat_name].loc[(region_endpointcells[treat_name]['track_id'] == trackid) & (region_endpointcells[treat_name]['movie'] == mov) & (region_endpointcells[treat_name]['experiment'] == experiment)]])
      filtered_tracks_region[treat_name].append(tracks_region[treat_name][i])
tracks_geo_region = filtered_tracks_geo_region
region_endpointcells = filtered_region_endpointcells

# with open('filtered_endpointcells.pkl', 'wb') as handle:
#   pickle.dump(region_endpointcells, handle, protocol=2)
