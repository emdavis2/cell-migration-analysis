...
def compile_data_tracks(treatment, min_track_length, region, pixel_size):
    ...
    for i in range(len(tracks_geo)):
        ...
        movie_num.append(tracks_geo[i]["movie"].iloc[0])
...
#Same function as above, but make all tracks the same length to compare
#returns ind_track_regions (indicies of tracks of interest), tracks_region (list of x,y cell coordinates for each track), tracks_geo_region (list of dataframes for each track for region of interest with track parameters)
def compile_data_track_cuttracklength(treatment, min_track_length, region, pixel_size):
    ...
    #remove tracks with less than 'mintracklength' points
    tracks = [track[:min_track_length] for track in tracks if len(track) >= min_track_length ]
    tracks_geo = [track[:min_track_length] for track in tracks_geo if len(track) >= min_track_length ]
